# `ado-cli` Skill Implementation Plan
...
**Tech Stack:** Markdown skill (`SKILL.md` with YAML frontmatter), Azure CLI (`az`) with the `azure-devops` extension, `git`. Manifests are JSON.

**Reference files (read before starting):**
- Spec: `docs/superpowers/specs/2026-06-17-ado-cli-skill-design.md`
- Existing skill to mirror: `plugins/azure-devops/skills/azure-devops/SKILL.md`
- Sibling skill for frontmatter/style: `plugins/sdd-kit/skills/create-decision/SKILL.md`

**Verification note:** This artifact is a behavioral doc, not runnable code, and there is no Azure access in this repo. "Tests" are therefore static checks: JSON validity (`python3 -m json.tool`), frontmatter presence, content coverage (`grep`), and a "no MCP references" check. `az` command syntax has been verified against Microsoft Learn / Azure CLI docs; runtime correctness is the user's to confirm against their server.

---

## File Structure

- **Create:** `plugins/sdd-kit/skills/ado-cli/SKILL.md` — the entire skill (one focused file, mirrors the existing skill's single-file structure).
- **Modify:** `plugins/sdd-kit/.claude-plugin/plugin.json` — extend `description` to mention `/ado-cli`.
- **Modify:** `.claude-plugin/marketplace.json` — update the `sdd-kit` entry `description`.
- **Modify:** `CLAUDE.md` (repo root) — add `ado-cli` under the `sdd-kit` section and to the repository structure tree.

---

## Task 1: Create SKILL.md — frontmatter + reference half (Step 0, Setup, Quirks, command map)

**Files:**
- Create: `plugins/sdd-kit/skills/ado-cli/SKILL.md`

- [ ] **Step 1: Create the skill file with the reference-half content**

Create `plugins/sdd-kit/skills/ado-cli/SKILL.md` with exactly this content:
...
4. Set CLI defaults so later commands are terse:
   ...
   ```
   Still pass `--org {organization} --project {project} --detect false` explicitly on
   every command below — defaults are a convenience, explicit args are the contract.
...
- [ ] **Step 2: Verify the file exists with valid frontmatter and no MCP references**

Run:
```bash
test -f plugins/sdd-kit/skills/ado-cli/SKILL.md && \
head -1 plugins/sdd-kit/skills/ado-cli/SKILL.md | grep -qx -- '---' && \
grep -q '^name: ado-cli$' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
grep -q '^allowed-tools: Bash, Read, Glob, Grep$' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
! grep -qi 'mcp__\|ado_get\|ado_post\|parse_ado_remote' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
echo "OK: frontmatter valid, no MCP refs"
```
Expected: `OK: frontmatter valid, no MCP refs`

- [ ] **Step 3: Verify quirks and setup sections are present**

Run:
```bash
grep -q '## Setup Check' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
grep -q '## Quirks' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
grep -q 'False compatibility warning' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
grep -q 'Emoji stripped' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
grep -q 'az devops invoke' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
echo "OK: setup + quirks + command map present"
```
Expected: `OK: setup + quirks + command map present`

- [ ] **Step 4: Commit**

```bash
git add plugins/sdd-kit/skills/ado-cli/SKILL.md
git commit -m "feat(ado-cli): add skill frontmatter, setup, quirks, and command map"
```

---

## Task 2: Append the five workflow sections to SKILL.md

**Files:**
- Modify: `plugins/sdd-kit/skills/ado-cli/SKILL.md` (append)

- [ ] **Step 1: Append the workflow sections**

Append exactly this content to the end of `plugins/sdd-kit/skills/ado-cli/SKILL.md`:
...
5. **Checkout the source branch** to explore the PR state: note current branch; `git stash -u` if needed; `git checkout origin/{sourceBranch} --detach` (or `{commitId} --detach` if the deleted-branch fallback was used).
...
1. **Find the PR:** `az repos pr list --repository {repo} --source-branch {currentBranch} --status active --org {org} --project {project} --detect false -o json`.
...
6. **Update the PR:** `az repos pr update --id {prId} --title "{newTitle}" --description "{newDescription}" --org {org} -o json`. Include only the flags for fields being changed.
...
- [ ] **Step 2: Verify all five workflows are present**

Run:
```bash
for w in "Pipeline Analysis" "PR Review" "PR Creation" "PR Update" "Changelog"; do
  grep -q "## Workflow: $w" plugins/sdd-kit/skills/ado-cli/SKILL.md || { echo "MISSING: $w"; exit 1; }
done && echo "OK: all five workflows present"
```
Expected: `OK: all five workflows present`

- [ ] **Step 3: Verify conventions carried over (German title schema, Asana, icons)**

Run:
```bash
grep -q 'Icon <Asana-Ticket-Id> Component' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
grep -q 'app.asana.com/0/search?q=ID-XXXXX' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
grep -q 'pull_request_template.md' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
grep -q 'Repository Mismatch Check' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
echo "OK: conventions carried over"
```
Expected: `OK: conventions carried over`

- [ ] **Step 4: Commit**

```bash
git add plugins/sdd-kit/skills/ado-cli/SKILL.md
git commit -m "feat(ado-cli): add the five Azure DevOps workflows via az CLI"
```

---

## Task 3: Update manifests and root CLAUDE.md

**Files:**
- Modify: `plugins/sdd-kit/.claude-plugin/plugin.json`
- Modify: `.claude-plugin/marketplace.json`
- Modify: `CLAUDE.md`

- [ ] **Step 1: Update the sdd-kit plugin description**

In `plugins/sdd-kit/.claude-plugin/plugin.json`, replace the `description` value with:
```
Toolkit for Spec-Driven Development. Provides skills for the full SDD workflow: Memory Bank management (Decision Records, Lessons Learned) and Azure DevOps via the Azure CLI. Currently available: /create-decision to document decisions, /create-lesson-learned to capture recurring patterns and pitfalls, and /ado-cli for Azure DevOps pipeline analysis, PR review/create/update, and changelogs via the az CLI (cloud and newest Server version).
```

- [ ] **Step 2: Update the marketplace entry for sdd-kit**

In `.claude-plugin/marketplace.json`, change the `sdd-kit` entry's `description` to:
```
SDD toolkit: /create-decision for decision records, /create-lesson-learned for recurring patterns, /ado-cli for Azure DevOps via the az CLI (cloud + newest Server)
```
...
Run:
```bash
python3 -m json.tool plugins/sdd-kit/.claude-plugin/plugin.json > /dev/null && \
python3 -m json.tool .claude-plugin/marketplace.json > /dev/null && \
grep -q 'ado-cli' plugins/sdd-kit/.claude-plugin/plugin.json && \
grep -q 'ado-cli' .claude-plugin/marketplace.json && \
echo "OK: JSON valid and mentions ado-cli"
```
Expected: `OK: JSON valid and mentions ado-cli`

- [ ] **Step 4: Update root CLAUDE.md**

In `CLAUDE.md`, under the `### sdd-kit` description in the "Plugins" section, add a bullet noting the new skill. Find the sdd-kit section and add, after its existing skill list:
```
...
```
Also add `ado-cli/` to the `sdd-kit` entry in the repository-structure tree near the top of `CLAUDE.md` (alongside `create-decision` and `create-lesson-learned`).

> If the `claude-md-improver` PreToolUse hook is active, it may also adjust `CLAUDE.md` at commit time — that is fine; just ensure the `ado-cli` mention is present after committing.

- [ ] **Step 5: Verify CLAUDE.md mentions the new skill**

Run:
```bash
grep -q 'ado-cli' CLAUDE.md && echo "OK: CLAUDE.md updated"
```
...
```bash
git add plugins/sdd-kit/.claude-plugin/plugin.json .claude-plugin/marketplace.json CLAUDE.md
git commit -m "docs(ado-cli): register skill in plugin, marketplace, and CLAUDE.md"
```
...
- [ ] **Step 1: Confirm the existing azure-devops plugin is untouched**

Run:
```bash
git diff --name-only main...HEAD | grep '^plugins/azure-devops/' && echo "PROBLEM: azure-devops changed" || echo "OK: azure-devops plugin untouched"
```
Expected: `OK: azure-devops plugin untouched`

- [ ] **Step 2: Confirm the skill is self-contained (no MCP, no leftover REST tool names)**

Run:
```bash
! grep -qiE 'mcp__|ado_get|ado_post|ado_patch|ado_delete|parse_ado_remote' plugins/sdd-kit/skills/ado-cli/SKILL.md && \
echo "OK: no MCP/REST tool references"
```
Expected: `OK: no MCP/REST tool references`

- [ ] **Step 3: Spec coverage check (manual)**

Re-read `docs/superpowers/specs/2026-06-17-ado-cli-skill-design.md` and confirm each item maps to delivered content: Step 0 detection (both URL forms), `az login` setup manual, both quirks, command map with invoke fallbacks, all five workflows with conventions carried over, manifests + CLAUDE.md updated, azure-devops untouched. Report any gap; if found, add a follow-up task rather than silently skipping.

- [ ] **Step 4: No commit needed** — verification only. If Step 3 surfaced fixes, commit them with a descriptive message.
