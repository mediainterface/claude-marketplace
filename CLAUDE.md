...
The skill set for MediaInterface's Spec-Driven Development (SDD) workflow — the skills we rely on across SDD (capturing the decisions and lessons behind a spec, gating the spec into a pull request, and handling the Azure DevOps work — PRs, work items, pipelines — along the way), not a general-purpose collection. It is mostly skills plus one hook (the spec→PR redirect); no MCP server.
